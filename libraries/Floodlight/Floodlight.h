...
static void output_bit(uint8_t, int);
static void write_command(uint8_t, uint32_t);
...
typedef struct Floodlight {
    ...
    void writeCommand();
    /*
    Floodlight(uint8_t dataPin, uint32_t command) : 
        pin(dataPin), currentCommand(command) {};
        */
}Floodlight;
/*
void setup()
...
static void write_command(uint8_t pin, uint32_t bitstream)
{ 
  digitalWrite(pin,0);
...
static void output_bit(uint8_t pin, int n)
{
  if (n == 0)
  {
    digitalWrite(pin,0);
